    ...
    async.waterfall([
        ...
        function (blocks, callback) {
            async.filter(blocks, function (block, mapCback) {
                apiInterfaces.rpcDaemon('getblockheaderbyheight', {
                    height: block.height
                    ...
                    block.orphaned = blockHeader.orphan_status === true ? 0 : 1;
        ...
        function (blocks, callback) {
            ...
            blocks.forEach(function (block) {
                if (block.orphaned = 1) return;
                log('info', logSystem, 'SET AS ORPHANED');
        ...
        function (blocks, callback) {
            ...
            blocks.forEach(function (block) {
                if (block.orphaned = 0) return;
                totalBlocksUnlocked++;
                log('info', logSystem, 'SET AS UNLOCKED');
